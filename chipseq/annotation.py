...
if __name__ == '__main__':
    ...
    for k in peaks.data:
        ...
        peak_sizes[k] = (dat.end - dat.start).values
        ...
        for c in dat.chrom.unique():
            ...
            for i, row in this_dat.iterrows():
                qry = tb.query(str(c), row.start, row.end)
                hit_gene = False
                hit_exon = False
                hit_tss = False
                for t in qry:
                    ...
                    if typ in SOURCES:
                        ...
                        if ftr == 'exon':
                            if 'exon_number "1"' in attr:
                                ...
                                hit_exon = True
                if hit_tss:
                    this_assignment['tss'] += 1.
                elif hit_exon:
                    this_assignment['exon'] += 1.
                elif hit_gene:
                    this_assignment['intron'] += 1.
                else:
                    this_assignment['intergenic'] += 1.

        peak_assignment[k] = this_assignment

    peak_assignment = pd.DataFrame.from_dict(peak_assignment)
    ...
    for ct in peaks.meta.chip_target.unique():
        ...
        this_dat = [peak_sizes[sn] for sn in snames]
        this_log_dat = [np.log10(peak_sizes[sn]) for sn in snames]
        this_x = range(len(this_dat))
        ax.violinplot(this_log_dat, this_x, widths=0.8)
        ax.plot(this_x, np.log10(np.array([np.median(t) for t in this_dat])), 'ko')
        ax.set_xticks(this_x)
        ax.set_xticklabels(snames, rotation=90)
        ax.set_ylim([2, 6])
        ax.set_ylabel('log10(peak size)')
        fig.tight_layout()
        fig.savefig(os.path.join(outdir, "peak_size_distribution_%s_%s.png" % (run_type, ct)), dpi=200)
    ...
    ss = peak_assignment.sum().loc[idx]
    pct = peak_assignment.divide(ss, axis=1)[idx.tolist()] * 100.
