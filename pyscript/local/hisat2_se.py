...
if __name__ == "__main__":
    ...
    required.add_argument("-x", "--index_dir", help="Directory of pre-computed STAR index", required=True)
    ...
    if args.out_dir is None:
        ...
        args.out_dir = os.path.join(args.read_dir, 'hisat2_alignment')
        if not os.path.exists(args.out_dir):
            os.makedirs(args.out_dir)
        sys.stderr.write("Output directory not specified, using default: %s\n" % args.out_dir)

    obj = hisat2.Hisat2SEBash(extra_args=extra, **args.__dict__)
    obj.create_script()
    obj.submit()