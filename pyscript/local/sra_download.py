#!/usr/bin/env python
import os
import argparse
import sys

# add root of project dir to the path
sys.path.append(os.path.dirname(__file__) + '/../../')

from pyscript import jobs


class LocalSraToFastqJob(jobs.ArrayJob, jobs.SraRunIteratorMixin, jobs.BashJobMixin):
    title = 'sra_to_fastq'
    required_args = ['project_id']
    parameters = [
        # format: (name as it appears in bash script, bash check or None)
        ('$ID', '! -z'),
        ('$SRR_ID', '! -z')
    ]
    core_cmd = "fastq-dump --gzip --split-files $SRR_ID -O {out_dir} {extra}"

    def prepare_submission(self, *args, **kwargs):
        self.setup_params(self.args['project_id'])


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    optional = parser._action_groups.pop()
    required = parser.add_argument_group('required arguments')

    optional.add_argument("-o", "--out_dir", help="Output directory", default=os.path.abspath('./'))
    required.add_argument("--project_id", help="SRA project ID (SRX or PRJNA)", required=True)

    # all extra args got to extra
    args, extra = parser.parse_known_args()

    obj = LocalSraToFastqJob(extra_args=extra, **args.__dict__)
    obj.create_script()
    obj.submit()