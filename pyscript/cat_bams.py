...
class CatBamRequirements(sge.ApocritaArrayJobMixin):
    @property
...
class CatBamsBase(jobs.ArrayJob):
    title = 'cat_bams'
    ...
    core_cmd = """
    outfn="$OUTDIR/${{ID}}.bam"
        samtools cat {extra} -o $outfn $BAMS
    """
...
class CatBamsMultilaneBartsBash(jobs.BashArrayJobMixin, CatBamsBase, jobs.BamBartsMultilaneRecursiveIteratorMixin):
    pass

class CatBamsMultilaneBartsApocrita(CatBamRequirements, CatBamsBase, jobs.BamBartsMultilaneRecursiveIteratorMixin):
    pass
...
def run(run_type):
    ...
    if run_type == 'bash_multilane_barts':
        cls = CatBamsMultilaneBartsBash
    elif run_type == 'apocrita_multilane_barts':
        cls = CatBamsMultilaneBartsApocrita
    else:
