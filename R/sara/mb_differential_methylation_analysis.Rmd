...
We'll need to filter probes in advance of testing for differential methylation. If we don't do this, our p values will all come out non-significant following the correction for multiple hypothesis testing.

We take a fairly simplistic approach, filtering based on the range and requiring a minimum variation across the samples.
...
for (cl in c('3021', '1299')) {
  ...
  for (cmp in the_names) {
    x <- this.dmps[[cmp]]
    plot(x$logFC, -log10(x$adj.P.Val), main=cl)
  }
}
