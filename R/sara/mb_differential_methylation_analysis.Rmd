...
meta <- read.csv(meta_fn, header=T, row.names = 1)
ix <- meta$subgroup == 'Group4'
meta <- meta[ix,]
...
densityPlot(as.matrix(beta.cavalli), sampGroups = meta$subtype_formatted, ylim = c(0, 7))

# the MDS plot seems to take forever, so disabling for now
if (F) {
  pal <- brewer.pal(3,"Dark2")
  plotMDS(m.cavalli, top=NULL, col=pal[factor(meta$subtype_formatted)])
  legend("topright", legend=levels(factor(meta$batch)), text.col=pal, bg="white")
}
...
```{r, cavalli_dm, echo=F}
  subtype <- meta$subtype_formatted
  signature <- meta$CHD7_signature
  groups <- as.factor(paste(subtype, as.character(signature), sep='.'))
  # design <- model.matrix(~0 + groups)
  
  # baseline is no signature, subtype alpha
  design <- model.matrix(~subtype+subtype:signature)

  fit <- lmFit(m.cavalli, design)
  ...
  # test: alpha, signature vs no signature
  this_fit <- contrasts.fit(fit, c(-1, 0, 0, 1, 0, 0))
  this_fit <- eBayes(this_fit, trend = T, robust = T)
  dmps.cavalli[['alpha_sign-nosign']] <- topTable(this_fit, number = Inf)
  writeLines(sprintf(
    "Comparison %s: %d DMPs with FDR < %.2f",
    'Alpha',
    sum(dmps.cavalli[['alpha_sign-nosign']]$adj.P.Val < alpha),
    alpha
  ))
