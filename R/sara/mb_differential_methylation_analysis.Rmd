...
Start by filtering the data in a similar way to our own.
...
```{r, cavalli_dm, echo=F}
  ...
  for (cont in colnames(cont.matrix)) {
    ...
    out_fn <- paste0("dmps_", cont, ".xlsx")
    write.xlsx(dmps.cavalli[[cont]], out_fn, colNames = T, rowNames = T)
    writeLines(sprintf(
      "Wrote the DMP results to file %s",
      out_fn
    ))
...
```{r cavalli_dm_plots}
  for (cont in names(dmps.cavalli)) {
    ...
    df <- m.cavalli.filt[probes,]
    df_melt <- melt(as.matrix(df))
    colnames(df_melt) <- c('probe', 'sample', 'beta')
    # add column with subtype, signature
    df_melt$subtype = meta[df_melt$sample, 'subtype_formatted']
    df_melt$signature = as.factor(meta[df_melt$sample, 'CHD7_signature'])
    ggplot(df_melt) + geom_boxplot(aes(x=subtype, y=beta, fill=signature)) + facet_wrap(~as.factor(probe))
  }
```
