...
meta.bowman$group <- paste(meta.bowman[,1], meta.bowman[, 3], sep='_')
...
```{r}
dgel <- DGEList(counts = data.bowman, samples = meta.bowman)
design <- model.matrix(~0 + group, data = meta.bowman)
dgel <- estimateDisp(dgel, design)
fit <- glmFit(dgel, design)
lrt <- glmTreat(fit, contrast=c(0, -1, 1, 0), lfc=logFC)
...
lrt <- glmTreat(fit, contrast=c(1, 0, 0, -1), lfc=logFC)
...
```
 ...
 Let's also generate a few boxplots for these genes, to visualise the actual difference in counts.
 
```{r}
# reformat so that each row is an observation
mtor_data <- as.data.frame(t(data.bowman[mtor_genes, rownames(meta.bowman)]))
mtor_data$group <- factor(meta.bowman$group, levels=c("monocyte_healthy", "bmdm_tumour", "mg_healthy", "mg_tumour"))
...
Interesting. The scatterplot shows several genes that we might expect to be tagged as DE in _both_ groups, but which only show up in one (or neither). This must be due to the estimation of dispersion? *TO DO*: check this.
...
write.csv(ta_bmdm.vs.h_bmdm, file=file.path(out.dir, "ta_bmdm_vs_h_monocyte.csv"))
write.csv(ta_mg.vs.h_mg, file=file.path(out.dir, "ta_mg_vs_h_mg.csv"))
```
