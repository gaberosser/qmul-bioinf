...
We start by looking at the number (or proportion) of DMPs by direction.
...
for (p in pids) {
  ...
  this_dmrcate <- dmrcate(this_annot, lambda = 1000, C=2, mc.cores = 8, betacutoff = 0.3)
...
The `bumphunter` package is conveniently implemented in the `ChAMP` package, so we'll use that.
...
for (p in pids) {
  res_bumphunter[[p]] <- champ.DMR(
  beta=cc_beta,
  pheno=group,
  compare.group = paste(p, c('GIC', 'iNSC'), sep='_'),
  arraytype='EPIC',
  method='Bumphunter',
  adjPvalDmr=alpha,
  cores=8
