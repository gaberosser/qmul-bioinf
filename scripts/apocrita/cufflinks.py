...
if __name__ == "__main__":
    ...
    optional.add_argument("--read_dir", help="Directory containing alignment files", default='./')
    optional.add_argument("-o", "--out_dir", help="Output directory")
    optional.add_argument("-p", "--threads", help="Number of threads", default='1')
    optional.add_argument("--library_type", help="Library type", default='fr-firststrand')
    ...
    if args.out_dir is None:
        ...
        args.out_dir = os.path.join(args.read_dir, 'cufflinks')
        if not os.path.exists(args.out_dir):
            os.makedirs(args.out_dir)
        sys.stderr.write("Output directory not specified, using default: %s\n" % args.out_dir)

    obj = sge.CufflinksSgeJob(extra_args=extra, **args.__dict__)
    obj.create_submission_script()
    obj.submit()

## TODO: TEST then remove all this
